        ...
        <form action="{{ isset($busride) ? route('admin.dashboard.busrides.update', $busride->id) : route('admin.dashboard.busrides.create') }}" method="POST" class="flex flex-col w-4/5 space-y-4">
            ...
            <div class="flex flex-row justify-between md:flex-row space-x-4 ">
                ...
                <div class="w-full">
                    ...
                    <input type="datetime-local" name="departure_time" value="{{ $busride->departure_time ?? '' }}" class="border border-gray-300 rounded-lg p-2 w-full" required>
                </div>
            </div>
            <div class="flex flex-row justify-between md:flex-row space-x-4 ">
                <div class="w-full">
                    <h3 class="text-lg font-bold">Tijd van aankomst</h3>
                    <input type="datetime-local" name="arrival_time" value="{{ $busride->arrival_time ?? '' }}" class="border border-gray-300 rounded-lg p-2 w-full" required>
                </div>
                <div class="w-full">
